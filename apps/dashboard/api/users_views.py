"""API v2 · Usuarios y permisos (F4).
    ...
    GET   /api/v2/users/<pk>/
    PATCH /api/v2/users/<pk>/            {roles?: [...], active?: bool, fullName?, email?}
...
puede quitarse a sí mismo 'Admin de sistema' (anti-lockout).
"""
...
class UserDetailView(_Base):
    ...
    def patch(self, request, pk):
        ...
        if "roles" in d:
            ...
            invalidos = [r for r in nuevos if r not in _ROLE_NAMES]
            if invalidos:
        ...
        if "fullName" in d:
            partes = (d["fullName"] or "").strip().split(" ", 1)
            u.first_name = partes[0][:150]
            u.last_name = (partes[1] if len(partes) > 1 else "")[:150]
